...
TOKEN = "8479817666:AAEnoq833aARTCf5L-awiQGeiNYJp3lsSH8"  # Remplacez par votre token
...
def build_main_menu():
    keyboard = [
        ...
        [
            ...
            InlineKeyboardButton("📈 Vibration +", callback_data="cmd_vibration_intensity_plus"),
            InlineKeyboardButton("📉 Vibration -", callback_data="cmd_vibration_intensity_minus"),
        ],
        [
...
async def button_click_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    command_map = {
        ...
        "cmd_vibration_intensity_plus": "/vibration_intensity +",
        "cmd_vibration_intensity_minus": "/vibration_intensity -",
    }
...
async def process_solde_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        pass
    command_queue.put(f"/solde {user_input}")
    await update.message.reply_text(
        "✅ Montant enregistré. Utilisez « Voir (10s) » pour l'afficher.",
        parse_mode="Markdown"
    )
    return ConversationHandler.END
...
async def process_prenom_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    command_queue.put(f"/prenom {user_input}")
    await update.message.reply_text(
        "✅ Prénom enregistré. Utilisez « Révéler (10s) » pour l'afficher.",
        parse_mode="Markdown"
    )
    return ConversationHandler.END
...
async def process_history_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_input = update.message.text.strip()
    command_queue.put(f"/history {user_input}")
    await update.message.reply_text(f"✅ Commande envoyée : `/history {user_input}`", parse_mode="Markdown")
    return ConversationHandler.END
...
async def process_image_url_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_input.startswith("http"):
        ...
        command_queue.put(f"/image {user_input}")
        await update.message.reply_text(f"✅ Commande envoyée : `/image {user_input}`", parse_mode="Markdown")
    else:
